        ...
        "content": [
            {
                ...
                "cache_control": {"type": "ephemeral", "ttl": "1h"} # 2026 新增: 可設置 TTL (5m 或 1h)
            }
        ]
    }
]
...
response = client.messages.create(
    ...
    max_tokens=1000,
    messages=messages,
    # ✨ 2026 新特性: Extended Thinking (擴展思維)
...
- **Prompt Caching 增強**: 支援 `ttl` 參數 (5m/1h)，可更靈活控制緩存生命週期。透過 `cache_control` 可以讓長對話歷史的成本降低 90%。
- **Extended Thinking**: 新增 `thinking` 參數，允許模型在回答前進行深度推理 (類似 o1)，需設置 `budget_tokens`。
- **最新模型**: **Claude Sonnet 4.5** (`claude-sonnet-4-5`) 是目前最強模型，支援超長 Context 和複雜 Agent 任務。

**特點：**
- **手動存儲**：開發者必須自建資料庫保存對話紀錄。
- **緩存優化**：Prompt Caching 解決了重複傳送歷史的成本問題。
- **深度推理**：Extended Thinking 提升了處理複雜邏輯的能力。
...
# ❌ 手動管理：需自行維護 Messages 列表
# Qwen/DashScope API (OpenAI 兼容模式) 是無狀態的
client = OpenAI(
...
- **Deep Thinking**: 支援 `enable_thinking=True` 參數 (通過 `extra_body` 傳遞)，可返回類似 o1/r1 的推理過程。
- **模型版本**: 最新模型包括 `qwen-plus-2025-04-28` 和 `deepseek-r1` (DashScope 託管版)。
- **兼容性**: 推薦使用 OpenAI 兼容接口 (`/compatible-mode/v1`)，遷移成本最低。
